    ...
    <div className="flex items-center justify-between px-14 fixed top-0 w-full flex w-full py-6">
      ...
      <div className="flex items-center justify-end">
        <button
          ...
          className="relative w-6 h-4 flex flex-col justify-between items-center group z-120 outline-none"
        >
          <span
            className={`
        ...
        <nav className={`
          ...
          bg-black w-full h-full duration-500 grid grid-rows-1 md:grid-cols-2
        `}>
            ...
            <div className="group grid grid-rows-3 gap-8">
              {mainNavLinks.map((links) => (
                <Link
                  key={links.href}
                  href={links.href}
                  className={`${spaceMono.className} text-6xl group-hover:text-[#a7a7a7] hover:text-white duration-300`}
                >
                  {links.text}
                </Link>
              ))}
